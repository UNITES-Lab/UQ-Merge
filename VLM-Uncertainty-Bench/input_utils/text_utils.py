...
def get_inputs(row, tokenizer, image_processor, model_name, model, conv_mode, dataset_name,
               perturbate=False, augmenter=None, prompt_type=None, text_prompts=None):
    ...
    if last_part_model_name.startswith('llava'):
        ...
        #got gtom repo
        if prompt_type is not "dummy_tokens":
        ...
        if perturbate:
            if prompt_type == "sys_msg":
                prompt = np.random.choice(text_prompts) + prompt
            elif prompt_type == "dummy_tokens":
                dummy_token = np.random.choice(text_prompts)
                if dummy_token["pos"] == "before":
                    qs = dummy_token["text"] + qs
                    conv = conv_templates[conv_mode].copy()
                    conv.append_message(conv.roles[0], qs)
                    conv.append_message(conv.roles[1], None)
                    prompt = conv.get_prompt()
                elif dummy_token["pos"] == "after":
                    qs = qs + dummy_token["text"]
                    conv = conv_templates[conv_mode].copy()
                    conv.append_message(conv.roles[0], qs)
                    conv.append_message(conv.roles[1], None)
                    prompt = conv.get_prompt()
                elif dummy_token["pos"] == "both":
