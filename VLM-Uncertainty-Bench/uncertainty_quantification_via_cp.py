...
def get_accuracy(test_result_data):
    ...
    for row in test_result_data:
        ...
        pred_answer = ALL_OPTIONS[np.argmax(row["logits"][:len(ALL_OPTIONS)])]
        # print(truth_answer, pred_answer)
...
def get_ce(result_data, norm):
    ...
    pred = torch.tensor(
        np.array(
            [softmax(row["logits"][:len(ALL_OPTIONS)]) for row in result_data]
        )
    )
...
def LAC_CP(cal_result_data, test_result_data, alpha=0.1):
    ...
    for row in cal_result_data:
        probs = softmax(row["logits"][:len(ALL_OPTIONS)])
        truth_answer = row["answer"]
...
def calculate_metrics(result_data, args, model_results):
    ...
    # print(test_result_data[0].keys())
    # print(test_result_data[0]["logits"])
    # print(test_result_data[0]["answer"])
    # exit()
    test_id_to_answer = {}
    for row in test_result_data:
...
def calculate_metrics_for_model(model_name, args):
    ...
    file_name = f'{args.result_data_path}/{model_name}'
    print(file_name)
...
def main(args):
    ...
    model_names = [f for f in os.listdir(args.result_data_path) if f.endswith(".pkl")]
    print(model_names)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cal_ratio", type=float, default=0.01,
